<script setup lang="ts">
</script>

<template>
  <div class="py-24 flex flex-col items-center">
    <h1 class="text-6xl font-bold mb-4 text-transparent bg-clip-text bg-gradient-to-br from-pink-400 to-purple-600">#Quote's</h1>
    <p class="mb-4 text-sm md:text-xl w-11/12 md:w-1/2 text-center text-gray-500">Kumpulan quote di dunia sebagai penyemangat dan nasihat. Dibuat untuk memeraiahkan Event Hacktoberfest 2021.</p>
    <div class="flex">
      <a class="mr-1" href="https://github.com/nyancodeid/quotes">
        <img src="https://img.shields.io/badge/github-quotes-brightgreen?logo=github&style=flat" alt="Repository">
      </a>
      <a class="mr-1" href="https://github.com/nyancodeid/quotes/pulls">
        <img src="https://img.shields.io/badge/PRs-welcome-brightgreen" alt="Repository">
      </a>
      <a class="mr-1" href="https://github.com/nyancodeid/quotes/pulls">
        <img src="https://img.shields.io/badge/github/issues-pr/nyancodeid/quotes" alt="Repository">
      </a>
      <a href="https://github.com/nyancodeid/quotes/issues">
        <img src="https://img.shields.io/badge/github/issues/nyancodeid/quotes" alt="Repository">
      </a>
    </div>
  </div>
</template>

<style>
</style>

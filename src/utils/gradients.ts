export const gradients: string[] = [
  "bg-gradient-to-br from-purple-500 to-indigo-500",
  "bg-gradient-to-br from-pink-500 to-red-500",
  "bg-gradient-to-br from-yellow-400 to-yellow-600",
  "bg-gradient-to-r from-purple-400 via-pink-500 to-red-500",
  "bg-gradient-to-br from-green-400 to-blue-500",
  "bg-gradient-to-br from-gray-700 via-gray-900 to-black dark:from-red-400 dark:via-pink-500 dark:to-indigo-500",
  "bg-gradient-to-br from-pink-400 to-purple-600",
  "bg-gradient-to-br from-pink-400 to-pink-600",
  "bg-gradient-to-br from-red-700 to-pink-600",
  "bg-gradient-to-br from-red-400 via-pink-500 to-indigo-500",
  "bg-gradient-to-r from-blue-500 to-blue-600",
  "bg-gradient-to-tl from-blue-400 to-blue-500",
  "bg-gradient-to-tl from-purple-600 to-pink-600",
  "bg-gradient-to-br from-purple-500 to-yellow-500"
];